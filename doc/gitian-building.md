...
*Setup instructions for a gitian build of CryptoVerification using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CryptoVerification
...
to cryptoverification-crypto.com.
...
- [Building CryptoVerification](#building-cryptoverification)
...
Clone the git repositories for cryptoverification and gitian and then checkout the cryptoverification version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cryptoverification-crypto/cryptoverification.git
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cryptoverification repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building CryptoVerification
----------------

To build CryptoVerification (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cryptoverification repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cryptoverification/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cryptoverification-crypto/cryptoverification
...
For example:
```bash
URL=https://github.com/crowning-/cryptoverification.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cryptoverification=${COMMIT} --url cryptoverification=${URL} ../cryptoverification/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cryptoverification=${COMMIT} --url cryptoverification=${URL} ../cryptoverification/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cryptoverification=${COMMIT} --url cryptoverification=${URL} ../cryptoverification/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cryptoverification-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cryptoverification-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cryptoverification-build.assert
```
...
[cryptoverification/gitian.sigs](https://github.com/cryptoverification-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
