...
#if defined(NDEBUG)
#error "CryptoVerification cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ucvcc) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zCVCC spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zCVCC is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zCVCC spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zCVCC without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zCVCC spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zCVCC spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcvcc");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zCVCC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 190000 && nHeight > 160000) {
		nSubsidy = 2.2 * COIN;
	}
	else if (nHeight <= 220000 && nHeight > 190000) {
		nSubsidy = 1.9 * COIN;
	}
	else if (nHeight <= 250000 && nHeight > 220000) {
		nSubsidy = 1.8 * COIN;
	}
	else if (nHeight <= 280000 && nHeight > 250000) {
		nSubsidy = 1.7 * COIN;
	}
	else if (nHeight <= 22179750 && nHeight > 280000) {
		nSubsidy = 1.6 * COIN;
	}
	else if (nHeight == 22179751) {
		nSubsidy = 1.25 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCVCCStake)
{
	int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from CryptoVerification
...
{
	RenameThread("cryptoverification-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZCVCCMinted()
{
...
void RecalculateZCVCCSpent()
{
	...
	while (true) {
		...
		//Remove spends from zCVCC supply
...
bool RecalculateCVCCSupply(int nHeightStart)
{
...
bool UpdateZCVCCSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		...
		RecalculateCVCCSupply(Params().Zerocoin_StartHeight());
	}

	//Track zCVCC money supply in the block index
	if (!UpdateZCVCCSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zCVCC supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCvccSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zCVCC spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zCVCC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zCVCC stake block is less than allowed start height", __func__);

	if (CZCvccStake* zCVCC = dynamic_cast<CZCvccStake*>(stake)) {
		CBlockIndex* pindexFrom = zCVCC->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zCVCC stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zCVCC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCVCC->GetValue()));
		if (nChecksum200 != zCVCC->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCVCC->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZCVCC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zCVCC fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> cvccInputs;
		std::vector<CTxIn> zCVCCInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zCVCCInputs.push_back(stakeIn);
			}
			else {
				cvccInputs.push_back(stakeIn);
			}
		}
		const bool hasCVCCInputs = !cvccInputs.empty();
		const bool hasZCVCCInputs = !zCVCCInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasCVCCInputs)
					...
					for (CTxIn cvccIn : cvccInputs) {
						if (cvccIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zCVCC inputs.
			if (hasZCVCCInputs) {
				for (CTxIn zCvccInput : zCVCCInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zCvccInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcvcc");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zCVCC mints and %d zCVCC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// CryptoVerification: We use certain sporks during IBD, so check to see if they are
