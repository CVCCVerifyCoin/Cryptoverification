...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (tx.IsZerocoinSpend()) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zcvccs that have been in mempool long
					//and higher priority to zcvccs that are large in value
					...
					// zCVCC spends can have very large priority, use non-overflowing safe functions
		...
		while (!vecPriority.empty()) {
			...
			// double check that there are no double spent zCVCC spends in this block or tx
			if (tx.IsZerocoinSpend()) {
				int nHeightTx = 0;
				...
				//This zCVCC serial has already been included in the block, do not add this tx.
				if (fDoubleSerial)
					continue;
			}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("CryptoVerificationMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake())
			pwalletMain->zcvccTracker->RemovePending(pblock->vtx[1].GetHash());
		return error("CryptoVerificationMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("CryptoVerificationMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("cryptoverification-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zCVCC with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					continue;
				}

				//Sign block with the zCVCC key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zCVCC key failed \n");
					continue;
				}
			}
		...
		LogPrintf("Running CryptoVerificationMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
