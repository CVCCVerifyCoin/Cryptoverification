...
#ifndef CryptoVerification_ZCVCCWALLET_H
#define CryptoVerification_ZCVCCWALLET_H
...
public:
    CzCVCCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCVCC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCVCC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CryptoVerification_ZCVCCWALLET_H
