...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
	...
	//It is VERY IMPORTANT that when this is being run and height < v2_start, then zCVCC need to be disabled at the same time!!
...
bool GenerateAccumulatorWitness(const PublicCoin &coin, Accumulator& accumulator, AccumulatorWitness& witness, int nSecurityLevel, int& nMintsAdded, string& strError, CBlockIndex* pindexCheckpoint)
{
	...
	while (pindex) {
		...
		/*// 10 blocks were accumulated twice when zCVCC v2 was activated
