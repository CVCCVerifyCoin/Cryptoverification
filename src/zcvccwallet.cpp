...
CzCVCCWallet::CzCVCCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcvcc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCVCCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCVCCSeed_deprecated()) {
                    LogPrintf("%s: Updated zCVCC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcvcc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcvcc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCVCCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCVCCCount(nCountLastUsed);
    else if (!walletdb.ReadZCVCCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCVCCWallet::Lock()
{
    seedMaster = 0;
}

void CzCVCCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCVCCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCVCC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCVCCWallet::LoadMintPoolFromDB()
{
...
void CzCVCCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCVCCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCVCCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcvccTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCVCCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCVCC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcvccTracker which also adds to database
    pwalletMain->zcvccTracker->Add(dMint, true);
...
void CzCVCCWallet::SeedToZCVCC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCVCCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCVCCWallet::GenerateDeterministicZCVCC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCVCCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCVCCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
