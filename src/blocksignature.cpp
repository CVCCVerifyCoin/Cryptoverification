...
bool CheckBlockSignature(const CBlock& block)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zCVCC or normal UTXO
     *  zCVCC: Each zCVCC has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the coinstake tx.
     */
    CPubKey pubkey;
    bool fzCVCCStake = block.vtx[1].IsZerocoinSpend();
    if (fzCVCCStake) {
