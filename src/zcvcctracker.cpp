...
CzCVCCTracker::CzCVCCTracker(std::string strWalletFile)
{
...
CzCVCCTracker::~CzCVCCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCVCCTracker::Init()
{
...
bool CzCVCCTracker::Archive(CMintMeta& meta)
{
...
bool CzCVCCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCVCCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCVCCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCVCCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCVCCTracker::GetSerialHashes()
{
...
CAmount CzCVCCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCVCCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCVCCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCVCCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCVCCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCVCCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCVCCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCVCCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCVCCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCVCCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCVCCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCVCCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCVCCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCVCCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCVCCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCVCCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCVCCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzcvcc from DB\n", __func__, listDeterministicDB.size());
    }
