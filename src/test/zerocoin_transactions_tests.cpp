...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zcvccTracker = unique_ptr<CzCVCCTracker>(new CzCVCCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZCVCC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZCVCC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
