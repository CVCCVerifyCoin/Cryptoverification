...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CVCC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCVCC send
enum ZerocoinSpendStatus {
    ZCVCC_SPEND_OKAY = 0,                            // No error
    ZCVCC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCVCC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCVCC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCVCC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCVCC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCVCC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCVCC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCVCC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCVCC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCVCC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCVCC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCVCC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCVCC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCVCC_SPENT_USED_ZCVCC = 14,                      // Coin has already been spend
    ZCVCC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCVCC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCvccBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCVCCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzcvccAuto) const;
    ...
    */
    ...
    CzCVCCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCVCCTracker> zcvccTracker;
    ...
    void setZWallet(CzCVCCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zcvccTracker = std::unique_ptr<CzCVCCTracker>(new CzCVCCTracker(strWalletFile));
    }

    CzCVCCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zCVCC reset */
    boost::signals2::signal<void()> NotifyzCVCCReset;
