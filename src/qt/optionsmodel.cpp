...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CVCC);
    ...
    if (!settings.contains("nAnonymizeCryptoVerificationAmount"))
        settings.setValue("nAnonymizeCryptoVerificationAmount", 1000);

    nAnonymizeCryptoVerificationAmount = settings.value("nAnonymizeCryptoVerificationAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCryptoVerificationAmount"))
        SoftSetArg("-anonymizecryptoverificationamount", settings.value("nAnonymizeCryptoVerificationAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cryptoverification.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCryptoVerificationAmount:
            return QVariant(nAnonymizeCryptoVerificationAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCryptoVerificationAmount:
            nAnonymizeCryptoVerificationAmount = value.toInt();
            settings.setValue("nAnonymizeCryptoVerificationAmount", nAnonymizeCryptoVerificationAmount);
            emit anonymizeCryptoVerificationAmountChanged(nAnonymizeCryptoVerificationAmount);
            break;
        case CoinControlFeatures:
