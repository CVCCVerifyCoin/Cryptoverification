...
#define URI_SCHEME "cryptoverification"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CryptoVerification address (e.g. %1)").arg("C6M1kT6PQaieDqNv2ajcTJtcfip2krta6a"));
...
{
    // return if URI is not valid or is no CryptoVerification: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::CVCC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert cryptoverification:// to cryptoverification:
    //
    //    Cannot handle this later, because cryptoverification:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CVCC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open cryptoverification.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoVerification.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CryptoVerification.lnk
...
{
    return GetAutostartDir() / "cryptoverification.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a cryptoverification.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CryptoVerification\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the cryptoverification app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add cryptoverification app to startup item list
