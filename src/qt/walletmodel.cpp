...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered cryptoverification address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CVCC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal cryptoverification:URI (cryptoverification:XyZ...?message=example)
            {
...
static void NotifyzCVCCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCVCCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCVCCReset.connect(boost::bind(NotifyzCVCCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCVCCReset.disconnect(boost::bind(NotifyzCVCCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcvccTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
