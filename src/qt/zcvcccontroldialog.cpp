...
#include "ui_zcvcccontroldialog.h"
...
std::set<std::string> ZCvccControlDialog::setSelectedMints;
std::set<CMintMeta> ZCvccControlDialog::setMints;

ZCvccControlDialog::ZCvccControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCvccControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCvccControlDialog::~ZCvccControlDialog()
{
    delete ui;
}

void ZCvccControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCvccControlDialog::updateList()
{
...
void ZCvccControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCvccControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCvcc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCvccControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCvccControlDialog::GetSelectedMints()
{
...
void ZCvccControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
