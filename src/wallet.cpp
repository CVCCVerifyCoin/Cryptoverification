...
#include "zcvccchain.h"

#include "denomination_functions.h"
#include "libzerocoin/Denominations.h"
#include "zcvccwallet.h"
...
/**
 * Fees smaller than this (in ucvcc) are considered zero fee (for transaction creation)
...
{
    return zcvccTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZCVCC = GetBoolArg("-zapwallettxes", false);
    if (fCheckZCVCC)
        zcvccTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zcvcc
            if (fCheckZCVCC && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zcvccTracker->GetMints(true);
    ...
    return zcvccTracker->GetBalance(false, false);
}
...
{
    return zcvccTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zcvccTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-cvccstake", true)) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CCvccStake> input(new CCvccStake());
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZcvccBalance = GetZerocoinBalance(false);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 CVCC
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 CVCC over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 CVCC+1 ( bit on if present )
            // bit 1 - 1000 CVCC+1
            // bit 2 - 100 CVCC+1
            // bit 3 - 10 CVCC+1
            // bit 4 - 1 CVCC+1
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 CVCC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 CVCC collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 CVCC.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 CVCC.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-cryptoverification-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZCVCC());
            ...
            //Mark mints as spent
            if (stakeInput->IsZCVCC()) {
                CZCvccStake* z = (CZCvccStake*)stakeInput.get();
...
void CWallet::AutoZeromint()
{
    ...
    // zCVCC are integers > 0, so we can't mint 10% of 9 CVCC
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zCVCC\n", nBalance);
        return;
    }

    // Percentage of zCVCC we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zCVCC (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zCVCC amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zCVCC amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zCVCC We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough CVCC to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zCVCC. Current percentage of zCVCC: %lf%%\n",
...
bool CWallet::CreateZCVCCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZCVCC(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZCVCCOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zcvcc output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Cvcc
    ...
    // Sign if these are cryptoverification outputs - NOTE that zCVCC outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZCVCC_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZCVCC_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZCVCC_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zCVCC privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZCVCC_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a CryptoVerification transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZCVCC_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), ZCVCC_BAD_SERIALIZATION);
            return false;
        }

        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zCVCC
            receipt.SetStatus(_("The coin spend has been used"), ZCVCC_SPENT_USED_ZCVCC);

            uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
            if (!zcvccTracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zcvccTracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zcvccTracker->UpdateState(meta))
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZCVCC_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), ZCVCC_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZCVCC_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zCVCC"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZCVCC_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCVCC transaction
    vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        setMints = zcvccTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zCVCC mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZCVCC_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zCVCC require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to cryptoverification address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZCVCC_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZCVCC_SPEND_OKAY); // Everything okay
...
string CWallet::ResetMintZerocoin()
{
    ...
    set<CMintMeta> setMints = zcvccTracker->ListMints(false, false, true);
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zcvccTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zcvccTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zcvccTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zcvccTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzcvccAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzcvccAuto ? "-autozcvccbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZCvccBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozcvccbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozcvccbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozcvccbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZCVCC_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZCVCC_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZCVCC_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zcvccTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZCVCC_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZCVCC_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zcvccTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zcvccTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZCVCC_SPEND_OKAY);  // When we reach this point spending zCVCC was successful
...
{
    CMintMeta meta;
    if (!zcvccTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zcvccTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zcvccTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zcvccTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zcvccTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zcvccTracker->Get(hashSerial);
    zcvccTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
