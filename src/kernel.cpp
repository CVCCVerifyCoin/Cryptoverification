...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZCvccStake(spend));
    } else {
        ...
        CCvccStake* cvccInput = new CCvccStake();
        cvccInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(cvccInput);
    }
