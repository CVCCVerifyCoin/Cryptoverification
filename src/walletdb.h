...
// Copyright (c) 2016-2018 The CryptoVerification developers
...
public:
    ...
    bool WriteZCVCCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCVCCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCVCCSeed_deprecated(uint256& seed);
    bool EraseZCVCCSeed();
    bool EraseZCVCCSeed_deprecated();

    bool WriteZCVCCCount(const uint32_t& nCount);
    bool ReadZCVCCCount(uint32_t& nCount);
